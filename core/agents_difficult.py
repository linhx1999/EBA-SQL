...
from core.utils_difficult import parse_json, check_letter, contain_value, add_prefix, load_json_file, extract_world_info, is_email, is_valid_date_column, extract_sql, detect_special_char, add_quotation_mark, get_matched_content_sequence, get_chosen_schema, extract_subquery
from core.bridge_content_encoder import get_matched_entries
LLM_API_FUC = None
...
class Logic_Driven_Selector(BaseAgent):
    ...
    def _data_prematch(self):
        ...
        if self.dataset_mode == "dev":
            matched_dict_path = './data/bird/dev/devSummary/match_content.json'
        else:
            matched_dict_path = './data/bird/test/testSummary/match_content.json'
        if os.path.exists(matched_dict_path):
            ...
            dir_name = os.path.dirname(matched_dict_path)
    ...
    def _get_summary(self):
        if self.dataset_mode == "dev":
            summary_file_path = "./data/bird/dev/devSummary/db_summary.json"
        else:
            summary_file_path = "./data/bird/test/testSummary/db_summary.json"

        if os.path.exists(summary_file_path):
            with open(summary_file_path, 'r', encoding='utf-8') as f:
                db_summary = json.load(f)
            print("Loaded db_summary from local file.")
            return db_summary
        ...
        for db_id in db_ids:
            db_schema, db_fk, db_pk, chosen_db_schem_dict, match_content, column_details = self._get_db_desc_str(
                db_id=db_id, extracted_schema={}, use_gold_schema=False, complete=True)
        ...
        dir_name = os.path.dirname(summary_file_path)

        if not os.path.exists(dir_name):
            os.makedirs(dir_name)

        with open(summary_file_path, 'w', encoding='utf-8') as f:
            json.dump(db_summary, f, ensure_ascii=False, indent=4)

        print("Generated and saved db_summary.")
        return db_summary
    ...
    def _prune(self,
        ...
        if matched_list != [] and self.dataset_name == "bird":
        ...
        prompt = selector_difficult.format(db_id=db_id, query=query, evidence=evidence, desc_str=db_schema, fk_str=db_fk, pk_str=db_pk, matched_list=matched_str)
...
class Logic_Driven_Decomposer(BaseAgent):
    ...
    def talk(self, message: dict):
        ...
        if message['send_to'] != self.name: return
        self._message = message
        query, evidence, schema_info, fk_info, matched_list = message.get('query'), \
            message.get('evidence'), \
        ...
        if self.dataset_name == 'bird':
            if matched_list != []:
                ...
                matched_str = 'No matched values.'
            decompose_template = decompose_template_bird_difficult
            prompt = decompose_template.format(query=query, evidence=evidence, desc_str=schema_info, fk_str=fk_info, pk_str=pk_info, detailed_str=column_details, matched_list=matched_str)
        else:
            decompose_template = decompose_template_spider
            prompt = decompose_template.format(query=query, desc_str=schema_info, fk_str=fk_info, pk_str=pk_info, detailed_str=column_details)

        ## one shot decompose(first)
