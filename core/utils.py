...
def parse_category(reply:str) -> str:
    start_keyword = "##category:"
    start_index = reply.find(start_keyword)

    if start_index != -1:
        start_index += len(start_keyword)
        category = reply[start_index:].strip()  # Remove extra quotation mark
        return category
    return ""
...
def extract_sql(a: str) -> str:
    ...
    if len(sql_list) > 0:
        ...
        final_sql = "error: No SQL found in refined SQL"
    return final_sql
